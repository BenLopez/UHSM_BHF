...
{ 
  ...
  EmulatorParametersMean$CorrelationLength <- function(X , n){
    return(0.4*(apply(X , 2 , max) - apply(X , 2 , min)) )
  }
...
}
{
  ...
  EmulatorParametersMax$X <- Training_X[Training_ImCrit[,2]<5 , -3]
  EmulatorParametersMax$Y <- Training_ImCrit[Training_ImCrit[,2]<5,2]
...
EmulatorParametersMax$MeanFunction <- function(X){
  ...
  {Xstar <- Validation_X[Validation_ImCrit[,2]<5 , -3]
   EmulatorOutput <- BE_BayesLinearEmulatorLSEstimates(xstar = Xstar ,EmulatorSettings = EmulatorParametersMax  )
  
    V_std <- var((Validation_ImCrit[Validation_ImCrit[,2]<5,2] - EmulatorOutput$E_D_fX)/sqrt(diag(EmulatorOutput$V_D_fX) ))
    R_sqM <- (1- var(Validation_ImCrit[Validation_ImCrit[,2]<5,2] - EmulatorOutput$E_D_MX)/var(Validation_ImCrit[Validation_ImCrit[,2]<5,2]))
    R_sq <- (1- var(Validation_ImCrit[Validation_ImCrit[,2]<5,2] - EmulatorOutput$E_D_fX)/var(Validation_ImCrit[Validation_ImCrit[,2]<5,2]))
  
  
    BE_PlotStdResiduals(Validation_ImCrit[Validation_ImCrit[,2]<5,2] , EmulatorOutput) + ggtitle('Standardised Residuals Mean Implausibility Threshold ')+ ggtitle(paste0('R Squared = ' , round(R_sq , 2) , ' Variance Std Resid = ' , round(V_std , 2) ) )
  }
}
