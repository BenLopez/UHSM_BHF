...
source('ADDAProcessDataReadmission.R')
#### Begin Data Analysis #####
...
tmp <- gsub('AFib' ,'Readmission' , colnames(PreOpCategoricalUnivariateResultsLatex))
tmp[12] <- 'L O CV AUC'
colnames(PreOpCategoricalUnivariateResultsLatex) <- tmp
tmp <- gsub('AF' ,'Readmission' , colnames(PreOpContinuousUnivariateResultsLatex))
tmp[12] <- 'LO CV AUC'
colnames(PreOpContinuousUnivariateResultsLatex) <- tmp
tmp <- gsub('AFib' ,'Readmission' , colnames(PostOpCategoricalUnivariateResultsLatex))
tmp[12] <- 'Left Out CV AUC'
colnames(PostOpCategoricalUnivariateResultsLatex) <- tmp
tmp <- gsub('AF' ,'Readmission' , colnames(PostOpContinuousUnivariateResultsLatex ))
tmp[12] <- 'LO CV AUC'
