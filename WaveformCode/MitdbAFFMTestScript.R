...
QSwidth = 12
{
  ...
  load(paste0(path , '\\' , subList , '\\Zip_out\\' , subList , '_', "AFTimes" , '.RData'))
...

{
  ...
  index <- which.max(abs(AFlocations[,1] - AFlocations[,2]) )
  MetaData = DP_CreateDummyMetaData(PatIndex2017 = PatIndex2017 , Name = PatientID , FirstNewAF = AFlocations[index , 1])  
  ...
  StarAndEndAF <- FM_ltafdbExtractStartandEndAF(RPeakData = RPeakData ,AFlocations = AFlocations  )
  StarAndEndAF <- StarAndEndAF[StarAndEndAF$End > min(RPeakData$RRCombined$t) & StarAndEndAF$End < max(RPeakData$RRCombined$t) , ]
  
  
    
  for(i in 1:dim(AFlocations)[1]){
  RRPlot <- RRPlot + geom_vline(xintercept = StarAndEndAF$Start[i] , col = 'purple')
  RRPlot <- RRPlot + geom_vline(xintercept = StarAndEndAF$End[i] , col = 'blue')
  }
...
for(PatientID in listAllPatients[3:length(listAllPatients)] ){
  {
    ...
    ECGs$ECGII$Value <- 100*ECGs$ECGI$Value
    RPeakData <- DP_LoadRpeaksfile( path , PatientID )
    load(paste0(path , '\\' , PatientID , '\\Zip_out\\' , PatientID , '_', "AFTimes" , '.RData'))
  ...
  
  {
    ...
    pdf(file = paste0("D:\\ltafdb_outputs\\Annotation" , PatientID , '.pdf') )
    print(RRPlot)
    dev.off()
    
    save(outputstruct , file = paste0("D:\\ltafdb_outputs\\HMOutput" , PatientID , '.RData'))
  }
}
