set.seed(1)
for(PatientID in listAllPatients[624:length(listAllPatients)] ){
  outputstruct <- list()
  ...
  source('FM_DecisionSupportSinglePatient.R')
  {
    ...
    outputstruct[[i+1]] <- cbind(RegularLogical, RegularLogical2, RegularyIrregularLogical,RegularyIrregularLogical2,BadDataLogical)
