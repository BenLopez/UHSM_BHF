...
#### Group Analysis #####

  {
    ...
    #save.image(file = 'AllDiscreteDataWorkSpaceRRT.RData')
    ...
    NamesofPostopVariables <- c( NamesofPostopContinuousVariables[c(-35,-36)] )
    ...
    {
      {
        ...
        x11(20,20)
        grid.arrange(phist , EmpericalProbabilityPlot , ROCplot , NPVPPVPlot + ylim(sum(AFLogical)/length(AFLogical),1)+ xlim((1-sum(AFLogical)/length(AFLogical)),1)  , ncol =2 , nrow = 2)
      }
    ...
    BC_PlotPairsFromTwoVariables(PreOpModelOutputs[AFLogical ==1,1:8] ,PreOpModelOutputs[sample(which(AFLogical ==0),sum(AFLogical)),1:8],
                                 alpha = 0.1 ,
    ...
    BC_PlotPairsFromTwoVariables(PostOpModelOutputs[AFLogical ==1,1:9] ,PostOpModelOutputs[sample(which(AFLogical ==0),sum(AFLogical)),1:9],
                                 alpha = 0.1 ,
    ...
    x11()
    SampleofPoints<- sample(which(AFLogical ==0),sum(AFLogical))
    ...
    BC_PlotPairsFromTwoVariables(Total[AFLogical ==1,] ,Total[sample(which(AFLogical ==0),sum(AFLogical)),],
