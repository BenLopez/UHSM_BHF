...
tmp <- ReHmOutput$f_x[(ReHmOutput$Implausability[,1] < ImThreshold2[2])&(ReHmOutput$Implausability[,2] < ImThreshold2[1]),]
...
{
ReIreHmOutput <- FM_HistoryMatchRRCulmativeDensity( PriorNonImplausibleSet = PriorNonImplausibleSetTotal,x = xTotal ,F_x =  F_total, f_x = f_total,  MD = MD_Total , RRtimes = RRtimes , Corr_sdhat = Corr_sdhat1, imthreshold  = ImThreshold1[2] , imthreshold2 = ImThreshold1[1] )
BC_PlotPairsFromThreeVariables(PriorNonImplausibleSetRegularyIreRegular[1:500,] , PriorNonImplausibleSetRegular[1:500,] , ReIreHmOutput$NonImplausibleSets[(ReIreHmOutput$Implausability[,1] < ImThreshold1[2])&(ReIreHmOutput$Implausability[,2] < ImThreshold1[1]) ,] )  
