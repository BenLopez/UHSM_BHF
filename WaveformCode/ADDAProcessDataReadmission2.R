POM_FindFirstGoodRecord <- function(PatientData){
  ...
  tmp <- as.POSIXct(dayOfRecord[DayOne[1]]) + 9*60*60
  output <- PatientData$time[which.min(abs(difftime( PatientData$time , (as.POSIXct(dayOfRecord[DayOne[1]]) + 9*60*60))))] #<- PatientData$time[DayOne[1]]
  return(output)
}

if(!file.exists('MasterDataReadmission.RData')){
  MasterData <- POM_ExtractFirstRecords( AllDataStructure )
  save(MasterData , file = 'MasterDataReadmission.RData')
}else{
  load('MasterDataReadmission.RData')
}
...
tmplogical <- (apply(as.matrix(MasterData$NewPseudoId) , 1 , function(X){substr(DP_ExtractOpCodeFromNewPatinetID(X) , start =nchar(DP_ExtractOpCodeFromNewPatinetID(X))  , stop = nchar(DP_ExtractOpCodeFromNewPatinetID(X) ))} ) != '3') 

MasterData <- MasterData[ tmplogical, ]
...
MasterData$AFLogical <- (apply(as.matrix(MasterData$NewPseudoId) , 1 , function(X){substr(DP_ExtractOpCodeFromNewPatinetID(X) , start =nchar(DP_ExtractOpCodeFromNewPatinetID(X))  , stop = nchar(DP_ExtractOpCodeFromNewPatinetID(X) ))} ) != '1')

MasterData$AFLogical <- MasterData$PseudoId %in% MasterData$PseudoId[MasterData$AFLogical]
MasterData <- MasterData[(apply(as.matrix(MasterData$NewPseudoId) , 1 , function(X){substr(DP_ExtractOpCodeFromNewPatinetID(X) , start =nchar(DP_ExtractOpCodeFromNewPatinetID(X))  , stop = nchar(DP_ExtractOpCodeFromNewPatinetID(X) ))} ) == '1') , ] 
